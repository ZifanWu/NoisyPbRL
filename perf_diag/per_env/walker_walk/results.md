...
**Part-3 prediction confirmed even at smoke scale:** mean R̂ in the negative-control `frequent_relabel` regime is **0.88** (close to the healthy R̂≈1), while in the positive `rare_relabel` regime it drops to **1.03** — clearly closer to the vanishing R̂=0 boundary. See [figs/Fig3.png](figs/Fig3.png), left panel: the violin for `rare_relabel` sits well below `frequent_relabel`, exactly as the spec predicts ("R̂ should be near-healthy under frequent relabeling, degrade as P_relabel grows").
...
run to populate the (FAR, lead) curve. In this pilot each run produced only ~8
...
|---|---|---|---|---|
| frequent_relabel | 6 | 1 | 2.23 | 0.877 |
| med_relabel | 6 | 0.667 | 2.47 | 1.03 |
| rare_relabel | 6 | 0.5 | 2.28 | 1.01 |
