...
**Part-3 prediction confirmed even at smoke scale:** mean R̂ in the negative-control `frequent_relabel` regime is **0.19** (close to the healthy R̂≈1), while in the positive `rare_relabel` regime it drops to **-0.16** — clearly closer to the vanishing R̂=0 boundary. See [figs/Fig3.png](figs/Fig3.png), left panel: the violin for `rare_relabel` sits well below `frequent_relabel`, exactly as the spec predicts ("R̂ should be near-healthy under frequent relabeling, degrade as P_relabel grows").
...
run to populate the (FAR, lead) curve. In this pilot each run produced only ~98
...
|---|---|---|---|---|
| frequent_relabel | 6 | 1 | 1.5 | 0.194 |
| med_relabel | 6 | 1 | 2.06 | -0.155 |
| rare_relabel | 6 | 1 | 2.2 | 0.101 |
...
This pilot is the **harness**, not the headline. Across 18 smoke runs
