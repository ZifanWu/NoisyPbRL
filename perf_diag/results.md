...
> 2 seeds, 2 regimes, reduced horizon.
...
**Part-3 prediction confirmed even at smoke scale:** mean R̂ in the negative-control `frequent_relabel` regime is **1.51** (close to the healthy R̂≈1), while in the positive `rare_relabel` regime it drops to **0.59** — clearly closer to the vanishing R̂=0 boundary. See [figs/Fig3.png](figs/Fig3.png), left panel: the violin for `rare_relabel` sits well below `frequent_relabel`, exactly as the spec predicts ("R̂ should be near-healthy under frequent relabeling, degrade as P_relabel grows").
...
run to populate the (FAR, lead) curve. In this pilot each run produced only ~5
...
|---|---|---|---|---|
| frequent_relabel | 2 | 1 | 1.35 | 1.51 |
| rare_relabel | 2 | 0.5 | 1.2 | 0.588 |
...
This pilot is the **harness**, not the headline. Across 4 smoke runs
(2 seeds × 2 regimes × 1 task),
