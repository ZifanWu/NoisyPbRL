...
For the `small_rm` slice, mean R̂ in `frequent_relabel` is **1.12** and mean R̂ in the positive relabel-stress slice is **1.00**. Interpret this only after checking Table 2: `frequent_relabel` is a valid negative control only if its `frac turned over` is near zero.
...
Across 4 runs, the headline claim is resolved only for slices with
